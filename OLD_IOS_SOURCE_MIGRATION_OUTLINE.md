...
| --- | ---: | ---: | ---: | ---: | ---: |
| Groups | 55 | 51 | 0 | 4 | 0 |
| Media/files/stickers | 54 | 41 | 9 | 4 | 0 |
...
| UI/customization/cross-platform | 29 | 21 | 6 | 1 | 1 |
| Search/organization/storage/notifications | 28 | 25 | 3 | 0 | 0 |
| Stories | 22 | 0 | 0 | 18 | 4 |
...
- Message history, load-older pagination, read refresh, and message-content consume state through old-iOS-matched `messages.readMessageContents` / `channels.readMessageContents` service calls. Channel post view/forward/reply refresh now uses old-iOS `messages.getMessagesViews`, group/channel read participant snapshots use old-iOS `messages.getMessageReadParticipants#31c1c44f`, reaction detail pages use old-iOS/server `messages.getMessageReactionsList#461b3f48`, and reaction picker cloud state uses old-iOS `messages.getRecentReactions`, `messages.getTopReactions`, `messages.setDefaultReaction`, `messages.clearRecentReactions`, and default reaction read from `help.getConfig` `Config.reactionsDefault`. The current native UI is still mock-driven, so this pass keeps the new capabilities at protocol/facade level.
- Text send, reply send, left-swipe reply, reply reference preview/jump, server-backed drafts with reply target restore, edit, delete, forward, reactions, supergroup pins, message links, URL taps, and mention/hashtag taps into chat search. Delete parity now follows old iOS peer splitting: private/basic groups use `messages.deleteMessages`, while channels/supergroups use `channels.deleteMessages`.
...
- Poll and todo messages now follow the old iOS/server MTProto contracts at protocol/facade level: native code builds `inputMediaPoll` / `inputMediaTodo`, parses `messageMediaPoll` / `messageMediaToDo`, sends votes, refreshes poll results, pages poll voters, toggles todo completions, and appends todo items. UI wiring stays deferred while the current UI branch is mock-driven.
- Global search across dialogs, contacts, and messages.
- Dialog list parity now keeps manual unread marks, pinned state, and muted state from dialog flags, marks chats read/unread through old-iOS peer-split `messages.readHistory` / `channels.readHistory` plus `messages.markDialogUnread`, clears history through old-iOS peer-split `messages.deleteHistory` / `channels.deleteHistory`, consumes per-message media/mention/reaction unread state through existing `messages.readMessageContents` and `channels.readMessageContents` service adapters, pins/unpins chats through existing `messages.toggleDialogPin`, reorders pinned chats through existing `messages.reorderPinnedDialogs`, counts manual marks in the Unread filter, loads the Archived filter with `messages.getDialogs(folder_id: 1)`, archives/unarchives chats through existing `folders.editPeerFolders`, and manages server chat folders through native `messages.getDialogFilters` / `messages.updateDialogFilter` / `messages.updateDialogFiltersOrder` / `messages.toggleDialogFilterTags` with include/exclude/pinned/category matching in the Chats tab.
...
- Shared media browser for media, files, links, GIF, voice, and music, using existing MTProto `messages.search` with `inputMessagesFilterPhotoVideo`, `inputMessagesFilterDocument`, `inputMessagesFilterUrl`, `inputMessagesFilterGif`, `inputMessagesFilterRoundVoice`, and `inputMessagesFilterMusic`; rows page by `offset_id`, tab counts come from old-iOS `messages.getSearchCounters`, old calendar buckets and sparse positions are protocol/facade-ready through `messages.getSearchResultsCalendar` and `messages.getSearchResultsPositions`, and rows jump back into the loaded chat thread.
...
- Profile, privacy summary, privacy base rules plus Always Allow/Never Allow peer exceptions through existing `account.setPrivacy`, blocked users management, notification scopes plus exception/sound/reset and ringtone protocol facades through existing `account.getNotifyExceptions`, `account.resetNotifySettings`, `account.getSavedRingtones`, `account.saveRingtone`, and `account.uploadRingtone`, storage summary, device reset, and account deletion through existing `account.deleteAccount`.
- Appearance, language, and support settings entries based on old `ThemeSettingsController`, localization list, and FAQ/support flow; wallpaper catalog/detail/save/install/reset is protocol/facade-ready through existing `account.*WallPaper*` MTProto, and language pack list/preview/full-pack/string/difference sync is protocol/facade-ready through existing `langpack.*` MTProto, while deeper theme assets, wallpaper UI wiring, and broader hard-coded copy localization remain.
...
1. Messaging parity
   ...
   - Shared media browser: media, files, links, GIF, voice, and music. Done for native MTProto filters, server counters through `messages.getSearchCounters`, old calendar buckets through `messages.getSearchResultsCalendar`, sparse positions through `messages.getSearchResultsPositions`, paging, jump-to-message, link-preview rows, and media/GIF grid phases; thumbnail-backed cells remain.
   ...
   - Reaction detail list. Done at protocol/facade level through existing server `messages.getMessageReactionsList#461b3f48`, parsing `messages.messageReactionsList#31bd492d` and `messagePeerReaction#8c79b63c`; UI remains mock-driven in this pass.
   - Poll/todo messages. Done at protocol/facade level through existing server `inputMediaPoll`, `inputMediaTodo`, `messages.sendVote`, `messages.getPollResults`, `messages.getPollVotes`, `messages.toggleTodoCompleted`, and `messages.appendTodoList`; UI remains mock-driven until the UI branch is ready.
...
2. Contacts and profiles
   ...
   - Address-book import now uses Contacts permission plus existing `contacts.importContacts`; imported-phone cleanup now uses existing `contacts.deleteByPhones`; share invite for non-HSgram contacts remains if still desired for App Store privacy.
   - Report/block flows connected to trust/moderation state; native protocol adapters now cover `account.reportPeer`, `account.reportProfilePhoto`, and old ContentReportScreen-style `messages.report` option/comment/result flow.

3. Groups and channels
   - Mark-read service parity now follows the old `SynchronizePeerReadState` split: private/basic groups use `messages.readHistory`, while channels/supergroups use `channels.readHistory`.
   - Clear-history service parity now follows the old `ManagedCloudChatRemoveMessagesOperations` split: private/basic groups use `messages.deleteHistory`, while channels/supergroups use `channels.deleteHistory#9baa9647`.
   - Message-content read service parity now follows the old `ManagedSynchronizeConsumeMessageContentsOperations` split: private/basic groups use `messages.readMessageContents`, while channels/supergroups use `channels.readMessageContents`.
   - Message deletion service parity now follows the old `ManagedCloudChatRemoveMessagesOperations` split: private/basic groups use `messages.deleteMessages`, while channels/supergroups use `channels.deleteMessages`.
...
4. Media and stickers
   ...
   - Sticker pack catalog identity now keeps old-iOS access hashes; install/uninstall, pack detail by id/access hash or short name, regular emoji sticker suggestions, custom emoji document resolution, custom emoji set listing/search, emoji keyword full/difference/language sync, saved GIF read/save/remove, archived listing for stickers/masks/emoji, featured-read, and recent sticker read/save/remove/clear are protocol/facade-ready through `messages.installStickerSet`, `messages.uninstallStickerSet`, `messages.getStickerSet`, `messages.getStickers`, `messages.getCustomEmojiDocuments`, `messages.getEmojiStickers`, `messages.searchCustomEmoji`, `messages.getEmojiKeywords`, `messages.getEmojiKeywordsDifference`, `messages.getEmojiKeywordsLanguages`, `messages.getSavedGifs`, `messages.saveGif`, `messages.getArchivedStickers`, `messages.readFeaturedStickers`, `messages.getRecentStickers`, `messages.saveRecentSticker`, and `messages.clearRecentStickers`. Faved stickers read empty-state through current server `messages.getFavedStickers`; faved write remains pending because the current server app does not expose `messages.faveSticker`.
   - Reaction picker with available/recent/top reactions plus default-read/default-set and clear-recent is protocol/facade-ready through existing server MTProto; UI remains mock-driven.
...
5. Settings and account security
   ...
   - Appearance/language/help/support screens. Native entries are in place with color scheme, text size, locale preference, FAQ/support links, protocol/facade-ready wallpaper catalog/detail/save/install/reset through existing `account.getWallPapers`, `account.getWallPaper`, `account.saveWallPaper`, `account.installWallPaper`, and `account.resetWallPapers`, and language pack list/preview/full-pack/string/difference sync through existing `langpack.getLanguages`, `langpack.getLanguage`, `langpack.getLangPack`, `langpack.getStrings`, and `langpack.getDifference`; remaining work is full old theme carousel UI, app icons, wallpaper UI wiring, and broad UI copy localization.
...
6. Notifications and extensions
   ...
   - Notification exceptions/sounds/reset and saved/uploaded ringtone state are protocol/facade-ready through existing `account.getNotifyExceptions`, `account.resetNotifySettings`, `account.getSavedRingtones`, `account.saveRingtone`, and `account.uploadRingtone`; UI remains mock-driven until the notifications UI branch is ready.
