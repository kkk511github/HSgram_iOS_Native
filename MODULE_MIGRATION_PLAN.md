...
- Remaining parity: Apple Sign In if product keeps it, server-supplied TOS handling, avatar upload during sign-up, support-email helper flows, account-count/premium-limit enforcement, review-account login-password/recovery E2E, and richer localized error mapping. Do not edit server source unless explicitly approved.
...
- Chat list and thread boundaries are split for folder-style filters, the thread container, composer/reply accessory panel, attachment menu shell, message bubble/reply reference, in-chat search views, forward target sheet, and message selection toolbar.
- Dialogs, old-style All/Unread/Contacts/Groups/Archived filtering, mark-read/mark-unread swipe actions, pinned/archived state, pinned drag reorder, text history, pagination, unread separator with first-unread initial scroll, text sending, edit/delete/forward/reactions/pins, replies, left-swipe reply, reply preview/jump, message links, URL taps, mention/hashtag taps into chat search, server drafts, saved messages, in-chat search, multi-select copy/forward/delete, APNS refresh, and foreground MTProto `updates.getState` / `updates.getDifference` refresh signaling exist, including affected-dialog refresh for common reaction/profile/member/notify/privacy/read/delete/draft/pin/folder/filter/channel/chat updates.
...
- Missing native client transport: current `HSAPIClient.sendMessage` was a typed REST facade call, but public `hsgram.cloud` does not expose `/v1`. Media and text sending must be mapped back to the existing HSgram-ios/Android/PC server protocol and media/dfs paths. Do not edit server source unless explicitly approved.
